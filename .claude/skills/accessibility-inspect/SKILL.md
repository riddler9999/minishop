...
The shared rules — severity, the no-proxy boundary, high-risk patterns, conformance, grounding — are in [`../accesslint-shared/methodology.md`](../accesslint-shared/methodology.md). Read it when a call needs judgment. The rules that always apply are below.
