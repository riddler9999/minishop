...
export default async function handler(req: any, res: any) {
  ...
  if (action === 'shop') return json(res, 200, {shop: {id: shop.id, name: shop.name, logoUrl: media(shop.logo_url), defaultDeliveryFee: shop.default_delivery_fee}});
  ...
  if (action === 'product') {
    ...
    return json(res, 200, {product: mapProductRow(data)});
  }
  if (action === 'products') {
    ...
    if (error) return json(res, 502, {error: 'Catalog unavailable'});
    return json(res, 200, {products: (data || []).map(mapProductRow), total: count ?? data?.length ?? 0});
  }
