...
function RevenueTrend({points}: {points: RevenuePoint[]}) {
  ...
  const coords = points.map((p, index) => {
    ...
    const y = padTop + chartHeight - (p.total / max) * chartHeight;
    return {x, y};
  });
  ...
  return (
    <div className="overflow-hidden rounded-[28px] border border-slate-200/80 bg-white p-4 shadow-sm sm:p-5">
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        <div className="flex h-10 items-center gap-2 rounded-2xl bg-slate-50 px-3 text-xs font-semibold text-slate-700">
          Last 7 days <ChevronDown className="h-4 w-4 text-slate-500" />
        </div>
      </div>
      <svg viewBox={`0 0 ${width} ${height}`} className="h-auto w-full" role="img" aria-label="Last 7 days revenue trend">
        ...
        {coords.map((p, index) => (
          <circle key={points[index].date.toISOString()} cx={p.x} cy={p.y} r="5" fill="#ff2f79" />
        ))}
        {points.map((p, index) => (
...
export default function Dashboard() {
  ...
  const currentOrders = useMemo(
    () => orders.filter((o) => new Date(o.created_at) >= currentStart),
    [orders, currentStart],
  );
  ...
  const actionOrders = orders.filter((o) => OPEN_STATUSES.includes(o.status as OrderStatus));
    ...
    () => [
      {
        ...
        sub:
          revenueDelta == null
            ? 'Last 7 days'
            : `${revenueDelta >= 0 ? '+' : ''}${Math.round(revenueDelta)}% vs previous 7 days`,
        icon: BarChart3,
      ...
      },
      {
        ...
        sub: actionOrders.length > 0 ? `${actionOrders.length} need your action` : 'All caught up',
        icon: ShoppingBag,
        tone: 'bg-cyan-50 text-cyan-500',
      },
      {
        key: 'products',
        label: 'Products',
  ...
  const revenuePoints = useMemo<RevenuePoint[]>(() => {
    return Array.from({length: 7}, (_, index) => {
      ...
      const total = orders
        .filter((o) => {
          const created = new Date(o.created_at);
          return created >= date && created < next && PAID_STATUSES.includes(o.status as OrderStatus);
        })
        .reduce((sum, order) => sum + (order.grand_total || 0), 0);
      return {date, total};
    });
  ...
  return (
    <div className="space-y-4 pb-24 sm:space-y-5 lg:pb-8">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex h-11 items-center gap-3 self-start rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 shadow-sm sm:self-auto">
          Last 7 days <ChevronDown className="h-4 w-4 text-slate-500" />
        </div>
      </header>
              ...
              return (
                <div key={stat.key} className="rounded-[26px] border border-slate-200/80 bg-white p-4 shadow-sm sm:p-5">
                  <div className="flex items-start gap-3">
                    <div className={`grid h-12 w-12 shrink-0 place-items-center rounded-[18px] ${stat.tone}`}>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className={`mt-1 text-[11px] leading-4 sm:text-xs ${stat.key === 'revenue' && revenueDelta != null && revenueDelta >= 0 ? 'font-semibold text-emerald-500' : 'text-slate-500'}`}>
                        {stat.sub}
                      </p>
                    </div>
                  </div>
      ...
      <section className="overflow-hidden rounded-[28px] border border-slate-200/80 bg-white shadow-sm">
        <div className="flex items-center justify-between px-4 py-4 sm:px-5">
          ...
          <Link to="/admin/orders" className="inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-slate-700 hover:text-pink-500">
              ...
              return (
                <li key={order.order_id}>
                  <Link to="/admin/orders" className="flex min-h-[76px] items-center gap-3 rounded-2xl px-2 py-3 transition hover:bg-slate-50">
                    <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-slate-50 text-slate-400">
                    ...
                    <span className={`hidden rounded-full px-3 py-1 text-[11px] font-semibold sm:inline-flex ${st.cls}`}>
                      {st.admin}
                    </span>
      ...
      <section className="overflow-hidden rounded-[28px] border border-slate-200/80 bg-white shadow-sm">
        <div className="flex items-center justify-between px-4 py-4 sm:px-5">
          ...
          <Link to="/admin/products" className="inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-slate-700 hover:text-pink-500">
            View all <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
            ...
            {lowStockPreview.map((product) => (
              <li key={product.id}>
                <Link to="/admin/products" className="flex min-h-[76px] items-center gap-3 rounded-2xl px-2 py-3 transition hover:bg-slate-50">
                  {product.image ? (
                    <img src={product.image} alt="" className="h-12 w-12 shrink-0 rounded-2xl border border-slate-100 object-cover" loading="lazy" />
                  ) : (
                    <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-slate-50 text-slate-400">
                      <Package className="h-5 w-5" />
                    </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 inline-flex rounded-full bg-pink-50 px-2.5 py-1 text-[11px] font-semibold text-pink-500">
