  ...
  return (
    <nav aria-label="Admin navigation" className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-3 pb-[max(10px,env(safe-area-inset-bottom))] pt-2 backdrop-blur lg:hidden">
      <div className="mx-auto grid max-w-lg grid-cols-4">
        {items
          .filter((entry) => entry.to !== '/admin/shipping')
          .map((entry) => {
            ...
            return (
              <NavLink
                ...
                className={({isActive}) =>
                  cx(
                    'flex min-h-[58px] flex-col items-center justify-center gap-1 rounded-2xl text-[11px] font-semibold transition',
                    isActive ? 'text-pink-500' : 'text-slate-500 hover:text-slate-900',
                  )
                }>
          ...
          <NavLink
            ...
            className={({isActive}) =>
              cx(
                ...
                isActive ? 'bg-pink-50 text-pink-600' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950',
              )
            }>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 px-1">
        <div className="grid h-11 w-11 place-items-center rounded-[16px] bg-pink-500 font-black text-white shadow-sm">
          {shopInitial(shop?.name)}
        </div>
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="truncate text-base font-black text-slate-950">{shop?.name ?? APP_NAME}</span>
            <PlanBadge />
          </div>
      ...
      <div className="space-y-1 border-t border-slate-200 pt-3">
        <Link to="/" className="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-slate-950">
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div
        ...
        tabIndex={-1}
        className="absolute inset-y-0 left-0 w-72 max-w-[85%] bg-white p-4 outline-none shadow-2xl">
        <div className="mb-4 flex justify-end">
          <button onClick={onClose} aria-label="Close navigation" className="grid h-11 w-11 place-items-center rounded-2xl text-slate-600 hover:bg-slate-50">
            <X className="h-5 w-5" />
          </button>
        </div>
        <Sidebar />
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fc] text-slate-950">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-[#f7f9fc]/95 px-4 py-3 backdrop-blur lg:ml-64">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          <button
            ...
            className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-slate-700 shadow-sm lg:hidden">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            ...
            <div>
              <p className="text-sm font-black text-slate-950">minishop</p>
              <p className="text-[11px] text-slate-500">Seller Console</p>
            </div>
          </div>

          <div className="ml-auto flex items-center gap-2">
            <button aria-label="Notifications" className="relative grid h-11 w-11 place-items-center rounded-2xl bg-white text-slate-700 shadow-sm">
              <Bell className="h-5 w-5" />
              <span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-pink-500" />
            </button>
            <div className="flex h-11 items-center gap-2 rounded-2xl bg-white px-2.5 shadow-sm">
              {shop?.logoUrl ? (
                ...
                <div className="grid h-8 w-8 place-items-center rounded-xl bg-cyan-50 text-sm font-black text-cyan-600">{shopInitial(shop?.name)}</div>
              )}
      ...
      <main className="lg:pl-64">
        <div className="mx-auto max-w-5xl px-4 py-5 sm:px-5 lg:py-7">
          <Outlet />
        </div>
      </main>
