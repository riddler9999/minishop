  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 px-1">
        <div className="grid h-11 w-11 place-items-center rounded-[16px] bg-brand-500 font-black text-white shadow-sm">{shopInitial(shop?.name)}</div>
      ...
      <div className="space-y-1 border-t border-slate-200 pt-3">
        <Link to={shop?.slug ? `/s/${shop.slug}` : '/demo'} className="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-semibold text-slate-600 transition hover:bg-brand-50 hover:text-brand-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400"><Store className="h-[18px] w-[18px]" />View store</Link>
        <button onClick={() => void signOut()} className="flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-slate-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400"><LogOut className="h-[18px] w-[18px]" />Sign out</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div ref={panelRef} role="dialog" aria-modal="true" aria-label="Navigation" tabIndex={-1} className="absolute inset-y-0 left-0 w-72 max-w-[85%] bg-white p-4 outline-none shadow-2xl">
        <div className="mb-4 flex justify-end"><button onClick={onClose} aria-label="Close navigation" className="grid h-11 w-11 place-items-center rounded-2xl text-slate-600 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400"><X className="h-5 w-5" /></button></div>
        <Sidebar />
      </div>
    </div>
  );
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-[#f7f9fc]/95 px-4 py-3 backdrop-blur lg:ml-64">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            <div className="grid h-10 w-10 place-items-center rounded-[14px] bg-brand-500 font-black text-white">{APP_INITIAL}</div>
            <div><p className="text-sm font-black text-slate-950">{APP_NAME}</p><p className="text-[11px] text-slate-500">Seller Console</p></div>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <span aria-label="Notifications" className="relative grid h-11 w-11 place-items-center rounded-2xl bg-white text-slate-700 shadow-sm"><Bell className="h-5 w-5" /><span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-brand-500" /></span>
            <div className="flex h-11 items-center gap-2 rounded-2xl bg-white px-2.5 shadow-sm">
              {shop?.logoUrl ? <img src={shop.logoUrl} alt="" className="h-8 w-8 rounded-xl object-cover" /> : <div className="grid h-8 w-8 place-items-center rounded-xl bg-brand-50 text-sm font-black text-brand-600">{shopInitial(shop?.name)}</div>}
