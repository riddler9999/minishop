...
export default function ProductCard({product, variant = 'default', className}: {product: Product; variant?: ProductCardVariant; className?: string}) {
  ...
  return (
    <article className={cx('group flex h-full flex-col overflow-hidden rounded-[20px] border border-[#f3e5ea] bg-white shadow-[0_10px_28px_rgba(88,52,64,0.10)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(88,52,64,0.14)]', className)}>
      <ShopLink to={to} className={cx('relative block overflow-hidden bg-[#f8eef2]', compact ? 'aspect-[4/4.3]' : 'aspect-4/5')}>
      ...
      <div className="flex flex-1 flex-col p-2.5 sm:p-3">
        ...
        <ShopLink to={to} className={cx('font-display line-clamp-2 font-semibold leading-snug text-[#2a2018] transition hover:text-[#be123c]', compact ? 'text-[13px] sm:text-sm' : 'mt-1 text-base')}>{product.name}</ShopLink>
        <div className="mt-1.5 flex flex-wrap items-center gap-2">
          <span className="font-sans text-[12px] font-bold text-[#251a12] sm:text-sm">{ks(price)}</span>
...
export function ProductCardSkeleton({compact = false}: {compact?: boolean}) {
  return <div className="overflow-hidden rounded-2xl border border-rose-100 bg-white shadow-[0_10px_28px_rgba(88,52,64,0.08)]"><div className={cx(compact ? 'aspect-square' : 'aspect-4/5', 'animate-pulse bg-[#f8eef2]')} /><div className="space-y-2 p-3"><div className="h-4 w-3/4 animate-pulse rounded bg-rose-50" /><div className="h-4 w-1/3 animate-pulse rounded bg-rose-50" /><div className="space-y-1.5 pt-2"><div className="h-8 animate-pulse rounded-lg bg-rose-50" /><div className="h-8 animate-pulse rounded-lg bg-rose-50" /></div></div></div>;
}
