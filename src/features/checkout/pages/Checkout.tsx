...
export default function Checkout() {
  ...
  if (!items.length) {
    return (
      <div className="commerce-page mx-auto max-w-3xl px-4 py-20 text-center">
        <p className="commerce-muted my">ခြင်းထဲတွင် ပစ္စည်းမရှိသေးပါ။</p>
        <ShopLink to="/products" className="commerce-primary mt-4 inline-block px-6 py-3 font-semibold">ဈေးဝယ်မယ်</ShopLink>
      </div>
    );
  }

  const label = 'commerce-label mb-1.5 block text-sm font-semibold';
  const input =
    'commerce-input w-full px-4 py-3 text-sm outline-none transition';
      ...
      <div className="commerce-checkout-grid grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.55fr)] lg:gap-8">
        <div className="space-y-5">
          ...
          <section className="commerce-panel">
            <h2 className="commerce-heading mb-4 flex items-center gap-2 font-semibold">
              <span className="commerce-step grid h-6 w-6 place-items-center text-xs">၁</span>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                <label htmlFor="checkout-township" className={label}>မြို့နယ် <span className="commerce-accent">*</span></label>
                <select
            ...
            {fee != null && (
              <p className="commerce-note my mt-3 px-3 py-2 text-sm">
                📦 {township} — ပို့ဆောင်ခ <span className="commerce-price font-bold">{ks(fee)}</span>
              </p>
            )}
            {live && shipErr && (
          ...
          <section className="commerce-panel">
            <h2 className="commerce-heading mb-4 flex items-center gap-2 font-semibold">
              <span className="commerce-step grid h-6 w-6 place-items-center text-xs">၂</span>
              ငွေပေးချေမှု
            </h2>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {PAYMENT_METHODS.map((m) => (
                <button
                  ...
                  className={cx('commerce-choice min-h-20 p-3 text-left transition', method === m.key && 'commerce-choice-active')}>
                  <span className="commerce-strong my block text-sm font-semibold">{m.label}</span>
                  <span className="commerce-muted my text-xs">{m.sub}</span>
                </button>
              ))}
            </div>

            <div className="commerce-note my mt-4 border border-dashed px-4 py-3">
              <span className="commerce-muted text-xs">{method === 'cod' ? 'အိမ်ရောက်မှ ပေးရမည့် ငွေ' : 'အခု လွှဲရမည့် ငွေ'}</span>
              <div className="commerce-price font-display text-2xl font-bold">{fee != null ? ks(grandTotal) : '—'}</div>
            </div>
              ...
              <>
                <div className="mt-3 space-y-2">
                  {providerAccounts.map((a) => (
                    <div key={a.phone + a.accountName} className="commerce-note flex items-center justify-between gap-2 border px-4 py-3">
                      <div className="min-w-0">
                        <p className="commerce-strong my truncate text-sm font-semibold">{a.accountName}</p>
                        <p className="commerce-price font-display text-base font-bold tracking-wide">{a.phone}</p>
                      </div>
                      <button
                        onClick={() => copy(a.phone, a.phone)}
                        className="commerce-primary inline-flex shrink-0 items-center gap-1 px-2.5 py-1.5 text-[11px] font-bold">
                        {copied === a.phone ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                        {copied === a.phone ? 'ကူးပြီး' : 'ကူးမယ်'}
                      </button>
                    </div>
                  ))}
                  {providerAccounts.length === 0 && <p className="commerce-muted my text-sm">အကောင့် မရရှိနိုင်ပါ။</p>}
                </div>
                ...
                <div className="mt-4">
                  <label htmlFor="checkout-reftail" className={label}>
                    Transaction နံပါတ်၏ နောက်ဆုံးဂဏန်း ၅လုံး ဖြည့်ပေးပါ <span className="commerce-accent">*</span>
                  </label>
                  <input
                    id="checkout-reftail"
                    className={input}
        ...
        <div className="h-fit space-y-4 lg:sticky lg:top-20">
          <div className="commerce-panel">
            <h2 className="commerce-heading font-display text-lg font-bold">အော်ဒါ အကျဉ်း</h2>
            ...
            <div className="commerce-divider my mt-4 space-y-2 border-t pt-4 text-sm">
              ...
              <div className="commerce-divider flex justify-between border-t pt-2 text-base">
                ...
                <span className="commerce-price font-display font-bold">{fee != null ? ks(grandTotal) : '—'}</span>
              </div>
            </div>
          </div>

          {err && <div className="commerce-error rounded-xl border p-3 text-sm">{err}</div>}

          <button
            disabled={!ready || submitting}
            onClick={submit}
            className="commerce-primary w-full min-h-14 px-6 py-4 font-bold transition disabled:cursor-not-allowed disabled:opacity-50">
            {submitting ? (
