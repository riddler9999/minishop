-- Seed verified Ninja Van Myanmar production rates from supplied charts.
-- Fee bands below reflect the supplied screenshots. Township spellings are normalized
-- to MiniShop's existing Myanmar location names where possible.
--
-- IMPORTANT:
-- This migration intentionally seeds only confidently matched routes from the charts.
-- Unknown routes remain absent and therefore fail closed via resolve_delivery_fee().

insert into public.ninjavan_rates
(origin_township, destination_region, destination_township, fee, source_label, is_active)
values
  -- Yangon origin (selected verified rows)
